...
import { MessageSquareText, X, Trash2 } from "lucide-react";
...
/**
 * Comment slipped between two tracks. A speech bubble sits in the gap;
 * hover shows a native preview, click opens the full note in a dialog
 * (portal — never clipped by the scrolling shelves).
 */
export function GapCommentBubble({ gap, isOwner, onDeleted }: GapCommentBubbleProps) {
  ...
  return (
    <>
      <button
        ...
        aria-label={`Comment between tracks ${gap.afterSourcePosition + 1} and ${gap.afterSourcePosition + 2}. Open to read.`}
        title={preview}
        className={cn(
      ...
      <Dialog.Root open={open} onOpenChange={setOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content
            ...
            aria-label={`Comment between tracks ${gap.afterSourcePosition + 1} and ${gap.afterSourcePosition + 2}`}
          >
            <div className="mb-2 flex items-center justify-between gap-2">
              <Dialog.Title className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#e8b34b]">
                Between Nº {gap.afterSourcePosition + 1} & {gap.afterSourcePosition + 2}
              </Dialog.Title>
